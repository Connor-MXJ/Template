...
namespace MXJ.Core.Utility.Extensions
{
     ...
     public class FilterWordExtension
    {
        ...
        /// 内存词典
        /// </summary>
        private WordGroup[] _memorylexicon = new WordGroup[(int)char.MaxValue];

        private string _sourctText = string.Empty;
        /// <summary>
        /// 检测源
        /// </summary>
        public string SourctText
        {
            get { return _sourctText; }
            set { _sourctText = value; }
        }
        ...
        private bool Check(string blackWord)
        {
            ...
            for (int i = 0; i < blackWord.Length; i++)
            {
                //特殊字符偏移游标
                int offset = 0;
                if (_nextCursor >= _sourctText.Length)
                {
                    break;
                }
                else
                {
                    ...
                    for (int y = _nextCursor; y < _sourctText.Length; y++)
                    {

                        if (!IsChs(_sourctText[y]) && !IsNum(_sourctText[y]) && !IsAlphabet(_sourctText[y]))
                        {
                            offset++;
                            //避让特殊字符，下位游标如果>=字符串长度 跳出
                            if (_nextCursor + offset >= _sourctText.Length) break;
                            _wordlenght++;
                    ...
                    if ((int)blackWord[i] == (int)_sourctText[_nextCursor + offset])
                    {
                        found = true;
                    }
                    else
	...
	///    string returnstr = "";//返回已经替换的敏感词
       ...
       /// returnstr = fw.Filter('*');//*为要替换成的字符
        ...
        {
            LoadDictionary();
            if (_sourctText != string.Empty)
            {
                char[] tempString = _sourctText.ToCharArray(); ;
